...
## *Author*: AAVA
## *Created on*: 
## *Description*: Data mapping for Bronze layer in Medallion architecture for Princess Cruise Yield Hunter. Defines one-to-one mapping from raw source tables to Bronze layer tables, preserving original structure and metadata. Includes updates for customer segment, order date transformation, and data quality rules.
## *Version*: 2
## *Updated on*: 
_____________________________________________
...
This mapping defines the ingestion of raw source data into the Bronze layer of the Medallion architecture in Snowflake. All attributes are mapped one-to-one, preserving original structure and metadata unless otherwise noted. Minimal transformations are applied as specified. No cleansing or business rules are applied at this stage except as noted in Data Quality Rules.
...
| Target Layer | Target Table      | Target Field      | Source Layer | Source Table      | Source Field      | Transformation Rule |
|--------------|------------------|-------------------|--------------|------------------|-------------------|---------------------|
| Bronze       | customer_data     | customer_segment  | Source       | customer_data     | segment           | 1-1 Mapping        |
| Bronze       | orders           | order_date        | Source       | orders            | order_date        | Convert YYYY-MM-DD to MM/DD/YYYY |

---

## Transformation Logic
- **order_date**: Convert from `YYYY-MM-DD` to `MM/DD/YYYY` format for consistency across systems.

---

## Data Quality Rules
- Ensure `customer_segment` values are non-null and match predefined categories.
- Validate `order_date` format post-transformation.

---

## Assumptions & Notes
- All mappings are one-to-one, preserving original structure and data types unless otherwise noted.
- No transformations, cleansing, or business rules applied in Bronze layer except as noted above.
...
- Mapping for `legacy_customer_id` has been removed as per requirements.
...
# OutputURL
https://github.com/DIAscendion/Princess_Cruise_Yield_Hunter/blob/main/DI_Snowflake_Bronze_Model_Data_Mapping_DIAS
